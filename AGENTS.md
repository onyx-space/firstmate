...
state/               runtime records and signals; gitignored
  ...
  .<id>.span-scan-cursor  per-task resumable progress for a status log's own span classification: classified position, file identity, span end, lines folded so far, folded open set, decision-origin map, and events collected so far, so one bounded round never redoes another's work; written only by fm-classify-lib.sh's bounded span scan while a round is in flight, removed when the round completes and by teardown, safe to delete (restarts the span from line 0)
