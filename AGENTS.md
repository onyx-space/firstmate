...
Keep every card under one of the three headings a backlog reader recognizes: a hand-written column-0 `## ` heading inside a card run is a passthrough section, so it hides each card under it from every listing by id until the cards move back under `## In flight`, `## Queued`, or `## Done`.
