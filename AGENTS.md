...
Sending that worker `/no-mistakes` is firstmate's fallback nudge for a worker that has not started its run, never a second start for one already driving it.
