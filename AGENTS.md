...
config/pr-forge-hosts  optional "<scheme>://<host>[:<port>] <token>" entries naming the instance-hosted HTTP forges this home watches; LOCAL, gitignored, operator-maintained, and NOT inherited by secondmate homes; absent means no such forge URL is accepted; see docs/configuration.md "HTTP forge hosts"
...
state/               runtime records and signals; gitignored
  ...
  <id>.claim-retired  one explicitly retired stale pool-slot claim, written beside the record whose claim it retires and honored by bin/fm-teardown.sh's slot-ownership guard and by that record's own teardown; the header owns its fields, and it outlives the record it retires
  ...
  inbox/             captain notes captured out of band by bin/fm-inbox.sh, including the voice handover's queued requests; each note appends one `check` wake and stays pending until acknowledged - a captain-authored note with `bin/fm-inbox.sh drain --ack <id>`, a notification-class note with the wake row it announced - either of which moves it to inbox/handled/, except that a merge notification for a repository this endpoint serves with a long-lived lane is dispatched to that lane by name and moves to inbox/dispatched/ instead (docs/voice-relay.md; docs/watcher-continuity.md)
  ...
  .<id>.span-scan-cursor.<start-offset>  per-span-start resumable progress for a status log's own span classification: classified position, file identity, span end, lines folded so far, folded open set, decision-origin map, and events collected so far, so one bounded round never redoes another's work and two callers classifying different spans of the same log never clobber each other; written only by fm-classify-lib.sh's bounded span scan while a round is in flight, removed when the round completes and by teardown, safe to delete (restarts that span from line 0)
...
Before writing or editing any pull request description, load the `pr-description` skill (`~/.agents/skills/pr-description/SKILL.md`), which is the single owner of that contract; a spawned worker receives the same pointer in its brief, rendered from `bin/fm-dod-lib.sh`.
...
3. For `check:`, act on the named poll result, including merges, Relay events, process-to-event source results, and captain inbox notes; a handled captain-authored note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate, while a notification-class note (`--source relay`) is archived by the wake-row acknowledgement itself - except a merge notification for a repository this endpoint serves with a long-lived lane, which the same acknowledgement dispatches to that lane by name and records as dispatched rather than handled (docs/watcher-continuity.md).
