...
The worker brief's `# Remote repository authority` section owns where a worker may push and open a PR: the clone's own fork by default, with an upstream or third-party repository needing the captain's explicit consent first and a clone whose `origin` points at an upstream parent stopping with a decision instead of opening a PR.
...
3. For `check:`, act on the named poll result, including merges, Relay events, process-to-event source results, and captain inbox notes; a handled captain-authored note is also acknowledged with `bin/fm-inbox.sh drain --ack <id>`, or it stays counted as still waiting for firstmate, while a notification-class note (`--source relay`) is archived by the wake-row acknowledgement itself (docs/watcher-continuity.md).
