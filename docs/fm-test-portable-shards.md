...
`parallel_max_ms` (the larger lane's hint sum), `parallel_imbalance_ms` (the
absolute difference between the two sums), and `parallel_unhinted` (members with
no parallel hint). Those sums are estimates from the hint table, not measured job
wall times. `tests/fm-test-run.test.sh` pins the lane health through them in
`test_portable_parallel_lanes_stay_duration_balanced`, which requires every member
to carry a hint and the two sums to differ by no more than five percent of the
larger one, so a lopsided or partly unhinted partition fails the guard instead of
silently running one lane into its job cap.
...
A passing guard reports the lane totals it checked as `serial=`, `serial_shards=`, and `serial_unhinted=`; read those instead of a snapshot taken when this document was written.
