...
- `tests/fm-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, the progress note, checkpoint refusals, rollback after a failed launch, the rebuild of a structurally gone endpoint, and the abort-time reclaim of an endpoint a rebuild created on tmux or flat Herdr - never one an ordinary relaunch adopted.
