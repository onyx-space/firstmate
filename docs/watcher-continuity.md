...
Once that fast bound is spent the retry does not stop: it drops to the constant cadence of `FM_WATCH_REARM_SLOW_MS` and keeps running while this session still owns the lock, because a session that owns lock and need but runs no cycle is a supervision outage.
...
After the configured retry bound is exhausted, it delivers the original wake with a typed continuity-restoration failure even if every successor arm hung without reporting readiness, and it keeps the slow retry above running until a successor is established.
