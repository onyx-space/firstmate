...
On that ordinary close path the fast bound is never terminal: once it is spent the retry drops to the constant cadence of `FM_WATCH_REARM_SLOW_MS` and keeps running while this session still owns the lock, because a session that owns lock and need but runs no cycle is a supervision outage.
The exhausted-bound failure is therefore surfaced once per failure episode rather than as a final report, and a delivered wake clears the episode so the next one gets its own fast budget and notice.
...
On the ordinary path an unexpected or failed close never stops: past the fast `FM_WATCH_REARM_RETRY_LIMIT` bound the retry drops to the `FM_WATCH_REARM_SLOW_MS` cadence and keeps running until a successor is established.
The restoration-exhaustion path is the one deliberate exception: when every successor arm hung without reporting readiness the bound is exhausted, the adapter delivers the original wake with a typed continuity-restoration failure, and it stops launching; recovery from that state is model-mediated, because the model receives the typed failure notification and calls `fm_watch_arm_pi` (the Pi and omp supervision-protocol entries own that step), and no further automatic launch happens.
That stop is deliberate rather than an oversight: the extension's own comment on the no-bare-arm rule records why a silent extra launch is the more dangerous choice, since it can hold a hung child that a later `fm_watch_arm_pi` reports as "unchanged" and so manufactures a state that looks supervised but is not.
...
When such a bounded acquire hits its deadline without a nameable live holder but a lock-shaped path remains - a stale owner record whose process is gone - the drain still skips just the step that acquire guards with an advisory that the lock is still held or being recovered, and keeps the acquire-failure verdict for a path that can never be a firstmate lock.
...
`tests/fm-wake-queue.test.sh`'s mixed-queue actor, stale-acknowledgement remedy, and presentation-deadline tests drive the real scripts: branch acknowledgement cannot swallow a main row, a concurrent main turn cannot present or acknowledge an active branch grant, a no-op stale acknowledgement names the current presented wake's exact command, live-holder presentation contention stays bounded and retriable, a queue-lock deadline without a named live holder stays contention instead of an unsafe lock, and acknowledgement locking remains blocking.
...
The source decides when the note leaves `state/inbox/` - archived, or dispatched to a lane that serves it - never whether it is presented.
...
One notification is not archived that way: a relay note reporting a merge in a repository this endpoint serves with a long-lived maintenance lane is dispatched into that lane's own session instead and recorded as dispatched, in `state/inbox/dispatched/`, because the lane is the reader the wake exists for and archiving it as handled would leave the lane unaware of the merge it maintains.
Which lane serves a repository is asked of `wire route`, whose answer reads the fleet's joined map, `<origmd clone>/ref/lanes.json` - which `scripts/owners.mjs` writes from the machine file's lane list and each lane's own checkout - together with this endpoint's own registration; only a lane registered on this endpoint counts, because a repository another machine maintains is that machine's own dispatch.
When that map is absent or silent about the repository, the same question is answered from the endpoint's machine file, whose `Long-lived maintenance lanes` line carries each lane's name and directory, plus the repository each lane's own checkout points at - so a lane checking out a different repository takes the archive path either way.
The dispatch addresses the lane by the registered name wire resolves, and the endpoint key it needs comes from wire's own answer (or, on the fallback, from wire's own config) rather than a second copy of the machine-key pairing.
A dispatch that cannot be made - no registered name, no wire on the machine, or a send whose own report does not put the payload in the mailbox, which wire can report as `unreadable` while still exiting 0 - leaves the note and its row exactly as a failed archival does: retryable and named on the acknowledgement's own output, never reported as delivered.
The row itself is consumed with the same acknowledgement as any other presented wake, because presenting it is what firstmate acknowledged; what the dispatch changes is where the note is recorded, so a reader of `state/inbox/` can tell a notification firstmate handled from one a lane took over.
Every automatic archival and every dispatch is named on the acknowledgement's own output with a count and the ids, so it can never look like a note going missing, and an archival or dispatch that cannot be written fails the acknowledgement before any row is consumed so the row and its note stay together for the next attempt.
...
Any surface that counts unconfirmed notes - the fleet board included - counts the notes still present in `state/inbox/`, so that count only falls when a note actually leaves it, archived or dispatched for real.
`tests/fm-wake-drain-inbox-ack.test.sh` drives the real pair: a captain-authored note survives its acknowledged row, a note queued through the producer's argv shape with no `--source` does too, a notification note follows it into `handled/`, a merge notification for a lane-served repository reaches that lane and lands in `dispatched/`, wire's routed lane dispatches with no machine-file lane to parse while a repository wire does not serve keeps the archive path and a map entry naming another machine is not second-guessed into a local dispatch, a merge is routed by its own token rather than a repository named in its title, a lane whose own checkout is another repository and a machine declaring no lane both keep the archive path, a lane declared with a symlinked machine file or a `~` directory is still resolved, an ordinary relay notification is never dispatched, a lane that serves the repository but cannot be reached keeps the note and its row retryable, a send wire accepted but did not put in the mailbox keeps it retryable too, a mixed batch does both at once, an explicit `--ack` still archives either source, and widening the notification class to every source turns the captain-authored case red.
