...
The lane list is the endpoint's machine file, whose `Long-lived maintenance lanes` line carries each lane's name and directory; the repository a lane serves is read from that directory's own checkout, never from the lane's name, so a lane checking out a different repository takes the archive path.
