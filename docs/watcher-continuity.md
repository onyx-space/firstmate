...
When such a bounded acquire hits its deadline without a nameable live holder but a lock-shaped path remains - a stale owner record whose process is gone - the drain still skips the round with an advisory that the lock is still held or being recovered, and keeps the acquire-failure verdict for a path that can never be a firstmate lock.
