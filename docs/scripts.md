...
| [`fm-dod-lib.sh`](../bin/fm-dod-lib.sh) | Own ship/scout worker role scope, ship definitions of done, the no-mistakes `--intent` contract, and the shared worker-facing PR-description block |
...
| `fm-watch-cycle-stats.sh` | Reduce the arm-owned cycle ledger to one watcher-cycle latency line (median, mean, max) plus an explicit ALERT past the median threshold |
| `fm-watch.sh`            | Singleton-safe watcher: absorb benign wakes, detect stalled local-secondmate and own wake queues, and exit on actionable ones |
...
| `fm-supervisor-target-lib.sh` | Resolve the shared supervisor target and backend for the daemon, launcher, and watcher |
