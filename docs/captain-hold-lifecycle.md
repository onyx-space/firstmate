...
A retained local-only delivery reaches the row only as that `Deliverable of the finished work:` body line, because `tasks-axi update` has no `--note` to carry it into a structured field, while the durable pending-close record carrying that note is retired when retention completes.
