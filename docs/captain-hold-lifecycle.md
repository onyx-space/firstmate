...
On 0 only the close changes: after cleanup and still under the task's own lock, teardown records one `Deliverable of the finished work: ...` line at the end of the task body, copies a canonical pull request or canonical `data/<id>/report.md` into the row's structured artifact fields, and runs `tasks-axi reopen`, so the row returns to Queued with its hold intact and remains on the appropriate Captain's Call or Charted Next decision surface instead of reading as work still under way.
The pending-close record teardown already stages before destructive cleanup carries that intent as a `mode=retain` line, so an interrupted cleanup replays the retention at the next session start through the same record, validator, and lock as an ordinary close and never closes the row; if the captain answers before replay, `answer` applies the record's retained artifact before closing the row - a canonical retained pull request or report through the row's structured artifact fields, and a retained note, including a pull URL `tasks-axi` refuses on `--pr`, as the same `Deliverable of the finished work: ...` body line the retention transition records - after which replay retires the record.
...
A retained local-only delivery - and a retained pull URL `tasks-axi` refuses on `--pr` - reaches the row only as that `Deliverable of the finished work:` body line, because `tasks-axi update` has no `--note` to carry it into a structured field, while the durable pending-close record carrying that note is retired when retention completes.
