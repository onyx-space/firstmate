...
FM_WATCH_REARM_RETRY_LIMIT=5   # Pi/OpenCode/omp adapter fast continuity-restoration retries before the cadence drops to FM_WATCH_REARM_SLOW_MS; exhaustion is surfaced once per failure episode, never a stop
FM_WATCH_REARM_SLOW_MS=60000   # Pi/OpenCode/omp adapter continuity retry cadence once the fast retry bound is spent; the retry keeps running while this session owns the lock so it is never left without a supervision cycle
