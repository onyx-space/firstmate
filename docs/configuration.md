...
FM_MAIN_WAKE_STALL_SECS=180        # minimum interval with no change of the oldest actionable row in THIS home's own state/.wake-queue (it advances as the session drains, and a queue reprovisioned under the same home starts a fresh interval at whatever sequence it restarts) before the watcher appends one keyed notification and exits actionably so this session drains what it was never woken for; a session provably inside an active turn (an exact native busy verdict, else the rendered busy signature of its own pane; unlike a mate there is no completed-turn marker for this session to age that proof against, so it holds for as long as the pane keeps rendering a busy signature) never escalates whatever this interval says, declared external-wait pause rows are excluded, and zero or invalid values use 180
