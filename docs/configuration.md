...
FM_PROC_ROOT_OVERRIDE=   # alternate /proc root for the repo's Linux process-identity reads, mainly for tests
...
FM_WATCH_ARM_RETIRE_TIMEOUT_MS=1000   # milliseconds Pi/OpenCode/omp wait for an unready successor arm to exit before abandoning retries
FM_WATCH_REARM_RETRY_BASE_MS=250   # Pi/OpenCode/omp adapter base delay for continuity restoration retries
FM_WATCH_REARM_RETRY_MAX_MS=4000   # Pi/OpenCode/omp adapter cap for exponential continuity retry delay
FM_WATCH_REARM_RETRY_LIMIT=5   # Pi/OpenCode/omp adapter fast continuity-restoration retries after an unexpected or failed close before the cadence drops to FM_WATCH_REARM_SLOW_MS; on that path exhaustion is surfaced once per failure episode and is never a stop, while exhausting it with every successor hung deliberately stops and hands recovery to the model
FM_WATCH_REARM_SLOW_MS=60000   # Pi/OpenCode/omp adapter continuity retry cadence after the fast retry bound is spent on an unexpected or failed close; that retry keeps running while this session owns the lock so it is never left without a supervision cycle, and it never covers the restoration-exhaustion stop
...
FM_CLASSIFY_SCAN_BUDGET_SECS=2     # seconds one round of a status log's own span classification may fold before it persists its progress and defers the rest to the next call, so a long log can never hold a watcher poll for minutes; clamped to 1..30, and a round that stops short says so (bin/fm-classify-lib.sh "bounded, resumable span scan")
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless the crew's own state admits them to the bounded FM_PAUSE_RESURFACE_SECS cadence instead (a declared wait, or a quiet external wait the watcher derives from its run state), while a live idle wait still surfaces once before that cadence bounds repeats
...
FM_PAUSE_RESURFACE_SECS=14400      # four hours between bounded rechecks of a declared external wait or verified captain-held transfer, between repeated new-hash stale alarms for an ordinary crew task whose wait is recorded (an open backlog captain call, or a delivered `done:` line), and between rechecks of a quiet external wait the watcher derives from the crew's own run state and anchors on its own first derivation rather than the status file's age; a structured until time can make an external-wait recheck occur sooner but cannot extend this bound; this includes a live idle pane after its first inconclusive stale wake and a live busy pane past FM_BUSY_TURN_MAX_SECS, while the away-mode daemon uses the same setting and ages a declared wait against the crew's own latest status line rather than pane busy state, and a derived quiet wait against its own recorded marker; a captain-held transfer is never rechecked while the away-posture record exists
