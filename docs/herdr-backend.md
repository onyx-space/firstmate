...
Durable task records are erased only once the exact pane is confirmed gone through its structured presence: after every close path, only a structured not-found response counts as gone, while a present or unknown result retains every record with a visible, retryable error. The one exception is the deferred projected close below: when the only reason the close was refused is the captain's own active tab and the read-only candidate check proves the session-start sweep will take that exact candidate, the record cleanup finishes and the close stays recorded in the retained journal.
...
Inside both locks it takes one exact snapshot, requires one unambiguous non-target focus and the exact title, token, tab, and pane shape, positively confirms the pane is agent-free, and reads Herdr's process information for the exact named-session pane.
An agent-free reading is either of the two `fm_backend_herdr_pane_agent_state` values that prove the pane's process has exited - no registration at all, or a registration that outlived its process (the crew shape's post-agent-exit reading) - and the process proof below is what actually licenses the close in both cases.
Its read-only `--candidate-ready <task-id>` mode asks the same question for bin/fm-teardown.sh's deferred close: it answers whether the sweep would take exactly that candidate once the only remaining obstructions are gone, mutating nothing and taking no locks, and relaxing exactly the two conditions a deferral means (the candidate's tab being the captain's active one, and the task record the asking teardown is about to remove).
bin/fm-teardown.sh defers a focus-refused close only when that mode answers yes, so a deferral is never handed a candidate this sweep would refuse on content grounds; anything unproven keeps every durable record.
...
Firstmate immediately revalidates the same journal, metadata absence, workspace title and token uniqueness, one-tab and one-pane topology, exact pane relationship, the agent-free reading, process proof, and non-target focus before calling the existing exact-pane focus-preserving close helper.
...
Every operational Herdr invocation goes through `fm_backend_herdr_cli`, which sets the environment and passes an explicit trailing `--session <name>`; the named server launch execs through `fm_backend_herdr_server_exec`.
...
A working Pi reads `unknown-busy` (a turn is running, so retrying after it ends is the remedy); a settled pane whose capture yields no selectable composer reads `unknown-shape` (waiting will not help, the screen must change).
Pending middle rows, missing identity, incomplete separator pairs, over-tall candidates, a blocked Pi, and the lower dead-shell rule still stay `unknown` or `pending`, and both new refusals remain `!= empty`.
Identity stays a lazy second read, consulted when a separator pair could change the verdict or a cursorless shape-selection refusal needs the pane's own state to name it.
...
Neither the stopped-server exception nor the stale-registration verdict widens husk detection; those paths still refuse an unreadable pane, and a `stale-agent` pane is reused by recovery and never closed as a husk, because the shell it holds may be a nested worktree shell. The session-start cleanup above is the one close path whose own `agent-free` set admits a stale registration, and only together with the lone idle childless shell proof.
