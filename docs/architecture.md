...
The same observation is applied to this home's own `state/.wake-queue`: a row here that stops being presented means this session was never woken to drain it, so the same no-progress interval, the same not-provably-in-an-active-turn gate, and the same keyed notification with its receipt and marker apply, timed by `FM_MAIN_WAKE_STALL_SECS`.
That observation runs after the foreign-queue one and reads, but never consumes or rewrites, either queue.
...
`tests/fm-wake-queue.test.sh` pins the own-queue no-progress wake, its fresh and advancing silence, its active-turn deferral, its recorded-episode receipt replay, and its place behind a due foreign-queue wake.
