...
That block points the worker at the separately installed `pr-description` skill, which owns the language discipline itself, carries the three-question pre-PR self-check that makes the discipline actionable without loading that skill, and renders nothing for `local-only` because that mode opens no PR.
