...
[`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh)'s `crew_stale_class` owns that classification and `pause_state_class` in [`bin/fm-watch.sh`](../bin/fm-watch.sh) maps it onto the watcher's decision, so only the working state ages the wedge timer above: a quiet external wait and a landed delivery take the `FM_PAUSE_RESURFACE_SECS` cadence instead, with a reason naming what is being waited on.
