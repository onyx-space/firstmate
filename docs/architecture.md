...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes the three-state readout does not absorb whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and attended captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
So a delivered ordinary crew task whose last line stays `done: PR ...` bounds repeated alarms from new pane hashes to the `FM_PAUSE_RESURFACE_SECS` cadence for the length of the captain's decision, whether firstmate has recorded a hold or the delivery line is the only record there is.
The first hash still alarms, each new hash inside that window is absorbed, and a new hash after the window re-surfaces the delivery; a terminal pane hash that never changes stays inert after its first alarm exactly as it did before this bound.
An unheld delivery is bounded from its `done:` line alone, so a delivery awaiting a merge word no longer spends one supervision cycle per pane tick.
That bound is deliberately limited to a delivered result: a blocker, a failure, and an inconclusive worker line are news the supervisor has not necessarily acted on, so they keep alarming on every new hash.
...
That absorb decision is three-state, not a single yes/no: the crew working now (a busy pane, or a run step the client reports as actively running or fixing), a quiet external WAIT (the pipeline's `ci` monitor, an active run whose step the client does not name, a run parked at a gate, a declared `paused:` wait, or a delivery that already landed), or neither.
[`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh)'s `crew_stale_class` owns that classification and `pause_state_class` in [`bin/fm-watch.sh`](../bin/fm-watch.sh) maps it onto the watcher's decision, with one deliberate asymmetry kept as a sealed known state rather than unified: on the non-terminal stale path only a busy pane ages the wedge timer above, so an advancing run step there takes the `FM_PAUSE_RESURFACE_SECS` cadence alongside a quiet external wait and a landed delivery, with a reason naming what is being waited on, while the terminal-status override still counts that advancing run step as actively working and arms the timer to supersede a stale captain-relevant line.
A run step the pipeline's own `last_activity` marks quiet has no progress behind it, so it is neither working nor waiting and surfaces at once.
Unknown, blocked, and failed readings keep surfacing exactly as before.
An active run step whose detail no version of the client vocabulary recognizes is a quiet external wait too, absorbed on the same bounded cadence rather than surfaced; only the client's own no-progress marker moves it out of that class.
...
Pi, omp, and OpenCode verify session-lock ownership and launch one singleton successor from their child-close handlers before delivering an actionable wake prompt, retrying a failed restoration on a bounded exponential delay that drops to a slow, never-stopping cadence while the session owns the lock.
...
The always-on watcher also uses that library's absorb classification on no-verb signals and first-sighting stale panes before status-log terminality is trusted, while the daemon maintains distinct wedge, declared-wait, and derived quiet-wait recheck cadences.
...
A quiet lane the same three-state readout names an external wait with no declaration on the log takes that bounded cadence too: the daemon re-derives the wait from the crew's own state when each window comes due, ages it against its own recorded marker, and returns the pane to ordinary wedge aging the moment the readout stops naming a wait or the pane reads busy.
In away mode, seen-status dedupe does not clear possible-wedge aging for nonterminal progress, so housekeeping still re-escalates an unchanged idle pane at the configured bound, except when that derived quiet-wait absorb explains its silence.
Away-mode housekeeping has no worktree-write deferral of its own, so while `state/.afk` exists a quiet crew that is writing its own worktree still escalates as a possible wedge at that bound; only that derived quiet-wait absorb, never the write evidence, can hold it off.
The daemon escalates captain-relevant events, plus a bounded recheck for a declared external wait that is still declared or for a quiet external wait the readout derives, as one batched, single-line digest using the canonical `away-supervisor` kind from `bin/fm-operational-input.sh` so firstmate can distinguish it structurally from real messages; captain-held transfers remain silent until return while the posture record exists.
...
Composer classification has one shared owner, `bin/fm-composer-lib.sh`: tmux, herdr, Zellij, Orca, and cmux contribute only a screen capture plus declarative styled, cursor, identity, and row capabilities, while the shared classifier owns every shape and the `empty`/`pending`/`pending-unproven`/`unknown`/`unknown-busy`/`unknown-shape` verdict.
`unknown-busy` and `unknown-shape` name the two refusal causes `unknown` used to flatten (a turn is running vs. the screen yielded no selectable composer); both are refusals, so only exact `empty` remains the positive proof.
...
`bin/fm-dod-lib.sh` is the one owner of that mode's definition of done and of the worker-facing PR-description block, each rendered both into a generated ship brief and into the ship instructions a promoted scout receives, so a promoted worker cannot be handed a weaker contract than a briefed one.
That block points the worker at the separately installed `pr-description` skill, which owns the language discipline itself, and carries the scope-aware pre-PR self-check: it makes the worker classify the PR, applies that skill's Chinese-first contract to the captain's own repos through three actionable questions, exempts third-party upstream PRs to their own conventions, and renders nothing for `local-only` because that mode opens no PR.
...
A pull request on an instance-hosted HTTP forge, `<scheme>://<host>[:<port>]/<owner>/<repository>/pulls/<n>`, is watched instead of merged: arming accepts it only for a host this home names in `config/pr-forge-hosts` ([configuration.md](configuration.md)), its state is read through that instance's own `/api/v1/repos/<owner>/<repository>/pulls/<n>` API rather than through a vendor CLI, and `bin/fm-pr-merge.sh` refuses it outright.
An armed watch is a byte copy of the tracked poll template, so an upgrade that changes that template re-anchors every already-armed watch through [`bin/fm-pr-poll-refresh.sh`](../bin/fm-pr-poll-refresh.sh) at the fleet-update convergence point, each home anchored against its own `bin/fm-pr-poll.sh` (including a remote mate's, refreshed by its own host after the fast-forward), and [`bin/fm-watch-arm.sh`](../bin/fm-watch-arm.sh) re-anchors again before it starts a watcher cycle; that refresh is idempotent, needs no network, never rewrites task metadata, and reports a task it cannot re-anchor for a hand re-arm instead of dropping it.
The first upgrade that carries this re-anchoring is still run by the prior release's bytes, which touch no poll, so for that one upgrade each home is re-anchored at its next supervision arm rather than inside the pass, and a merge landing before that arm is observed at that arm rather than on the check that follows the update.
The GitLab path merges only after one live read of the merge request confirms it is open, mergeable, conflict-free, with blocking discussions resolved and a successful pipeline at the current head, and it binds the merge to that verified head; recorded metadata is never the authority for those conditions because a rebase leaves it stale.
...
The proof admits two release readings for a contested slot, both proven on the shared copy and on the slot's own processes rather than on endpoint liveness alone: a single uncontested live claimant, and a claimant whose rival claim Treehouse's own pool record of the slot's current owner shows began before that owner started.
It refuses in every other combination, including two gone records and two claims no owner record separates.
When a state rather than a contradiction leaves the process half of both readings unmakable - the occupant's pane leader can no longer be named, or a process rooted in the slot descends from no live endpoint - the one supported way through is an explicit, recorded retirement of the stale claim, never an implied one and never a relaxed proof.
