...
[`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh)'s `crew_stale_class` adds the readout behind a silent pane: the crew working now (a busy pane, or a run step the client reports as actively running or fixing), a quiet external WAIT (the pipeline's `ci` monitor, an active run whose step the client does not name, a run parked at a gate, a declared wait), or neither. It is consumed by the crew-state no-progress marker, which reports a run step with no activity behind it as no-progress rather than as an active step. How the watcher's stale and wedge decisions should split those states is an open design question: this release keeps its existing classification there, so a quiet external wait is still absorbed on the same terms as an actively running step.
