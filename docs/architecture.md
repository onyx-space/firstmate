...
That pointer block points the worker at the separately installed `pr-description` skill, which owns the language discipline itself, and renders nothing for `local-only` because that mode opens no PR.
