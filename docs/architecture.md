...
[`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh)'s `crew_stale_class` adds the readout behind a silent pane: the crew working now (a busy pane, or a run step the client reports as actively running or fixing), a quiet external WAIT (the pipeline's `ci` monitor, an active run whose step the client does not name, a run parked at a gate, a declared wait, or a delivery that already landed), or neither. It reads the `no recent step activity` marker `bin/fm-crew-state.sh` appends, which reports a run step with no activity behind it as no-progress rather than as an active step, and it has no production caller in this release. How the watcher's stale and wedge decisions should split those states is an open design question: this release keeps its existing classification there, so a quiet external wait is still absorbed on the same terms as an actively running step.
...
The same observation is applied to this home's own `state/.wake-queue`: a row here that stops being presented means this session was never woken to drain it, so the same no-progress interval, the same not-provably-in-an-active-turn gate (an exact native busy verdict, else this home's own rendered busy signature, with no completed-turn marker for this session to age that proof against, unlike a mate), and the same keyed notification with its receipt and delivered marker apply, timed by `FM_MAIN_WAKE_STALL_SECS`.
That observation runs after the foreign-queue one and never consumes or rewrites an existing row in either queue, though the own-queue episode appends its keyed notification into the queue it is observing.
...
`tests/fm-wake-queue.test.sh` pins the own-queue no-progress wake, its fresh and advancing silence, its active-turn deferral, its re-delivery of an episode whose wake was lost before delivery, and its place behind a due foreign-queue wake.
