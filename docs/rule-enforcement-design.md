...
A PreToolUse hook can refuse these *at the moment*, because the harness knows the tool, its arguments, and the turn it is in; a turn-end hook arrives only after the call, so it can block the turn end or append a correction rather than refuse the call.
...
| A written artifact lands in a declared write root | the target path, against the declared roots | **PreToolUse / harness write gate** (existing: `~/code/origmd/extensions/write-scope.ts`, the pi `tool_call` guard that reads the list out of the injected file; read-back: `~/code/origmd/scripts/check.mjs`, write routing) | a write outside the roots is refused by name; the read-back fails on a root this endpoint lacks or a bullet declaring a second root the refusal never reads |
