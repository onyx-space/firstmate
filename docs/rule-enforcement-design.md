...
- The glyph rule (`a reply and its thinking carry no ✓, ✗ or Hmm marks`) was broken about 23,058 times: 21,889 in thinking, 944 in replies, 225 inside tool arguments.
...
These are marked **not mechanizable** below, with the smallest human practice that catches them, and no gate is built for them.
...
| 规则 | 可判形式 | 落点 | 验收怎么证 |
|---|---|---|---|
| A reply, its thinking, and its tool arguments carry no `✓` / `✗` / `Hmm` | the glyphs, in the finished turn's own text, matched as bytes | **Stop hook** (pi: the turn-end extension; other harnesses: their turn-end hook) appends one corrective line; **session-log counter** in the fleet's check reports the rate | replay a recorded reply that carried a `✗`: the corrective line appears in the model's next context; the checker's count for that session drops to zero on a re-run of the fixture |
...
| Information, a question and a report are not a dispatch | the turn's trigger text (captain message) plus the presence of a state-changing call in the same turn | **PreToolUse**, turn-scoped: the seatbelt marks a `查`/`研究`/report-shaped turn read-only and refuses write, dispatch and card changes in it | replay a recorded read-only turn with a write injected → refused; an ordinary turn is untouched |
...
| A written artifact lands in a declared write root | the target path, against the declared roots (already enforced by the harness's write tool refusal, verified in this session) | **PreToolUse / harness write gate** (existing) | already proven: a write outside the roots is refused by name; the design adds only the missing half below |
| The write-root list and the harness's refusal agree | the same list, read from both sides | **CI checker** in the fleet's check: the injected rule's roots and the harness gate's roots must be the same set | change one side only → the checker fails naming both files; the current pair passes |
...
| A watched repository's merge reaches the lane that maintains it | the lane delivery's own machine-readable facts (`wire` reports `mailbox: present` and the `lane=<registered name>:<delivery id>` marker), not the sender's belief | **CI checker / measurement** on the delivery ledger, not a gate: the captain's current A/B (direct delivery against the code-path dispatch) decides which channel is authoritative | the A/B's four cells, with "the lane session itself read it" as the deciding cell; the surviving channel is then enforced by skipping the other on that marker |
