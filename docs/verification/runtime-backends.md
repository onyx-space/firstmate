...
A stale-registration pane is never a husk: create, reclaim, and presentation recovery keep refusing it, and only recovery reuses it; the session-start cleanup is the one close path that admits it, as an `agent-free` reading licensed only together with the lone idle childless shell proof.
