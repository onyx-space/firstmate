...
- **Composer-state guard** - `inject_msg` reads the full `empty`/`pending`/`pending-unproven`/`unknown`/`unknown-busy`/`unknown-shape` verdict from `fm_backend_composer_state` and injects only when it is affirmatively `empty`.
...
Captain-relevant events, plus a bounded recheck of a declared external wait that is still declared or of a quiet external wait the readout derives from the crew's own state, escalate to firstmate's context as one pre-read, single-line, batched digest.
...
- `stale` for a quiet lane whose own current state is an external wait the shared three-state readout derives - its run still advancing, the pipeline's ci monitor or a gate, or work that already landed - self-handles on the same `FM_PAUSE_RESURFACE_SECS` recheck cadence with no declaration on the status line and no terminal captain verb there.
  The daemon re-derives that wait from the crew's own state when each window comes due and ages it against its own recorded marker, so a churning pane cannot restart the window.
  A busy pane, a run step with no progress behind it, an unreadable read, and a crossed busy-turn wedge escalation all keep their existing wedge routing instead of being downgraded into this wait.
...
- `stale` with a terminal status or bare legacy captain-relevant line -> escalate.
  ...
  If the pane is still idle past `FM_STALE_ESCALATE_SECS` (default 240s), housekeeping escalates it as a possible wedge, unless the crew's own state names an external wait (the quiet-external-wait bullet above).
