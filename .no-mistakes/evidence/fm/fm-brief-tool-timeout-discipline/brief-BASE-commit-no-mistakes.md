...
1. First action: create your branch: `git checkout -b fm/t-base`
...
2. Stay inside this worktree; outside it you may write only the paths this brief names: this task's own data directory (`/tmp/fm-base-home.7iBHXM/data/t-base/` - evidence and artifacts), the instruction inbox (`/tmp/fm-base-home.7iBHXM/state/t-base.inbox/`, including its `handled/` directory), and the status file (`/tmp/fm-base-home.7iBHXM/state/t-base.status`), plus, in a `--herdr-lab` brief, the Herdr session state its helper commands manage. Never the primary checkout.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-base-home.7iBHXM/state/t-base.status'`
...
6. If a decision belongs above the implementation worker (product choices, destructive actions),
   ...
   For a no-mistakes ask-user gate specifically, escalate all ask-user findings as one event plus one snapshot file, using that same shape even when the gate holds only a single ask-user finding: write only the ask-user findings, verbatim and unparaphrased (id, severity, file, line, description, authority), to `/tmp/fm-base-home.7iBHXM/data/t-base/nm-<run>-findings.txt`, then report the gate with
   `needs-decision [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=/tmp/fm-base-home.7iBHXM/data/t-base/nm-<run>-findings.txt`
...
Firstmate steers you through durable message files in '/tmp/fm-base-home.7iBHXM/state/t-base.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-base-home.7iBHXM/state/t-base.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-base-home.7iBHXM/state/t-base.inbox'/NNN.msg '/tmp/fm-base-home.7iBHXM/state/t-base.inbox'/handled/`.
...
# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/tmp/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md`, follow `/tmp/bin/fm-ensure-agents-md.sh`'s self-governance contract in the same pass.
