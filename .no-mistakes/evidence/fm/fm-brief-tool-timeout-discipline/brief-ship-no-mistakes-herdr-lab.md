...
1. Set `HERDR_LAB_HELPER='/Users/onyx/.no-mistakes/worktrees/9573b29b9316/01M29CXKPWBHWNJZ2WQH38C2XQ/bin/fm-herdr-lab.sh'` and generate the session name with `HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name t-herdr-ship)`.
...
1. First action: create your branch: `git checkout -b fm/t-herdr-ship`
...
2. Stay inside this worktree; outside it you may write only the paths this brief names: this task's own data directory (`/tmp/fm-brief-live.SxsFVw/home/data/t-herdr-ship/` - evidence and artifacts), the instruction inbox (`/tmp/fm-brief-live.SxsFVw/home/state/t-herdr-ship.inbox/`, including its `handled/` directory), and the status file (`/tmp/fm-brief-live.SxsFVw/home/state/t-herdr-ship.status`), plus, in a `--herdr-lab` brief, the Herdr session state its helper commands manage. Never the primary checkout.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-brief-live.SxsFVw/home/state/t-herdr-ship.status'`
...
6. If a decision belongs above the implementation worker (product choices, destructive actions),
   ...
   For a no-mistakes ask-user gate specifically, escalate all ask-user findings as one event plus one snapshot file, using that same shape even when the gate holds only a single ask-user finding: write only the ask-user findings, verbatim and unparaphrased (id, severity, file, line, description, authority), to `/tmp/fm-brief-live.SxsFVw/home/data/t-herdr-ship/nm-<run>-findings.txt`, then report the gate with
   `needs-decision [key=nm-<run>-<step>]: ask-user findings=<id1>,<id2>,... file=/tmp/fm-brief-live.SxsFVw/home/data/t-herdr-ship/nm-<run>-findings.txt`
...
Firstmate steers you through durable message files in '/tmp/fm-brief-live.SxsFVw/home/state/t-herdr-ship.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-brief-live.SxsFVw/home/state/t-herdr-ship.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-brief-live.SxsFVw/home/state/t-herdr-ship.inbox'/NNN.msg '/tmp/fm-brief-live.SxsFVw/home/state/t-herdr-ship.inbox'/handled/`.
