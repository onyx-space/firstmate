...
1. First action: create your branch: `git checkout -b fm/s-localonly`
...
# Rules
1. Never push to any remote and never open a PR. Work only on your `fm/s-localonly` branch; firstmate handles the merge into local `main`.
2. Stay inside this worktree; outside it you may write only the paths this brief names: this task's own data directory (`/tmp/fm-prdesc.2Tk7Av/home/data/s-localonly/` - evidence and artifacts), the instruction inbox (`/tmp/fm-prdesc.2Tk7Av/home/state/s-localonly.inbox/`, including its `handled/` directory), and the status file (`/tmp/fm-prdesc.2Tk7Av/home/state/s-localonly.status`), plus, in a `--herdr-lab` brief, the Herdr session state its helper commands manage. Never the primary checkout.
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '/tmp/fm-prdesc.2Tk7Av/home/state/s-localonly.status'`
...
Firstmate steers you through durable message files in '/tmp/fm-prdesc.2Tk7Av/home/state/s-localonly.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/fm-prdesc.2Tk7Av/home/state/s-localonly.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/fm-prdesc.2Tk7Av/home/state/s-localonly.inbox'/NNN.msg '/tmp/fm-prdesc.2Tk7Av/home/state/s-localonly.inbox'/handled/`.
...
# Project memory
If `AGENTS.md` or `CLAUDE.md` already exists, or if this task produced durable project-intrinsic knowledge, run `/Users/onyx/.no-mistakes/worktrees/9573b29b9316/01M27PNZYH0HBMPSN55HJ07K1Z/bin/fm-ensure-agents-md.sh .` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project `AGENTS.md`, follow `/Users/onyx/.no-mistakes/worktrees/9573b29b9316/01M27PNZYH0HBMPSN55HJ07K1Z/bin/fm-ensure-agents-md.sh`'s self-governance contract in the same pass.
...
The task is complete only when committed on your branch `fm/s-localonly`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if `main` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append `done: ready in branch fm/s-localonly` to the status file and stop.
