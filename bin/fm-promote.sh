...
6. These ship instructions supersede the scout delivery rules, the scout-time artifact-placement rules, the scout-time remote repository authority section, and the report-based Definition of done. Everything else in your original instructions carries over unchanged: the status protocol; the instruction inbox and its acknowledgement; the escalation rules, including ask-user; and every safety rule.
$PROMOTION_ASK_USER_BLOCK
...
EOF
  printf '\n'
  fm_remote_authority_block "$MODE"
  printf '\n'
