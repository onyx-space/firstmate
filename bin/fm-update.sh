...
# The re-exec handoff above hands the rest of the run to a second process whose
# own ff_target already reports the advance as done, so that marker is a second
# way in; without it the handoff would skip the rebind the advance made necessary.
if [ "$FF_STATUS" = "updated" ] || [ -n "${FM_UPDATE_REEXEC:-}" ]; then
