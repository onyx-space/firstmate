...
# slot-owner claim beside the slot, written by bin/fm-spawn.sh at the moment the
# slot is taken and dropped here once it is genuinely returned. A claim naming
# another task is proof of reassignment, so teardown warns, names the claimant,
# and finishes only this record's own cleanup while every step that would read or
# touch that slot is skipped: no process kill under it, no landed-work inspection
...
# The slot-owner claim is a second, independent proof of the same thing, and it
# is composed with the readings above rather than replacing them: whichever proof
# shows the slot is no longer this record's causes the same skip-the-slot
# cleanup, a claim that cannot be read refuses exactly like an unprovable release
# reading does, and neither one relaxes --force.
...
cleanup_firstmate_home_children() {
  local home=$1 sub_state child_meta child_id child_t child_wt child_slot child_proj child_kind child_home child_backend child_orca_worktree_id child_return_rc child_busy_gen child_owner_rc
  sub_state="$home/state"
  [ -d "$sub_state" ] || return 0
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      if [ "$child_owner_rc" -eq "$TEARDOWN_SLOT_REASSIGNED_RC" ]; then
        :
      elif [ -n "$child_slot" ] && teardown_claim_is_retired "$child_meta" "$child_slot"; then
