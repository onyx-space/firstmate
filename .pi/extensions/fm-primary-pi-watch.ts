...
export default function (pi: ExtensionAPI) {
  ...
  function scheduleRetry(owner: SessionGeneration, message: string, predecessorArmPid: string): void {
    ...
    if (exhausted && !owner.retryFailureSurfaced) {
      ...
      surfaceFailure(owner, `watcher: FAILED - Pi extension could not restore watcher continuity after ${retryLimit} retries; it keeps retrying on a slow cadence so this session is never left without a supervision cycle\n${message}`);
    }
